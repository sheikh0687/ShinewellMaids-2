...
struct ApiBookingList : Codable {
    
    let result : [ResBookingList]?
    let message : String?
    let status : String?
    ...
    init(from decoder: Decoder) throws {
        ...
        result = try values.decodeIfPresent([ResBookingList].self, forKey: .result)
