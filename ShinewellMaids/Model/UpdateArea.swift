...
//  ShinewellMaids
//
//  Created by Techimmense Software Solutions on 17/07/23.
//

import Foundation

struct ApiUpdateArea : Codable {
    
    let result : ResUpdateArea?
    let message : String?
    let status : String?
    ...
    init(from decoder: Decoder) throws {
        ...
        result = try values.decodeIfPresent(ResUpdateArea.self, forKey: .result)
        message = try values.decodeIfPresent(String.self, forKey: .message)
        status = try values.decodeIfPresent(String.self, forKey: .status)
    }
}

struct ResUpdateArea : Codable {
    let id : String?
    let store_id : String?
