...
//  ShinewellMaids
//
//  Created by Techimmense Software Solutions on 05/07/23.
//

import Foundation

struct ApiAddBooking : Codable {
    
    let result : ResAddBooking?
    let message : String?
    let status : String?
    ...
    init(from decoder: Decoder) throws {
        ...
        result = try values.decodeIfPresent(ResAddBooking.self, forKey: .result)
        message = try values.decodeIfPresent(String.self, forKey: .message)
        status = try values.decodeIfPresent(String.self, forKey: .status)
    }
}

struct ResAddBooking : Codable {
    let id : String?
    let user_id : String?
