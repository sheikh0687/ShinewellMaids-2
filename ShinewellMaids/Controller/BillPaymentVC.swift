...
class BillPaymentVC: UIViewController {
    ...
    @IBOutlet var listnerExpiryDate: MaskedTextFieldDelegate!
    ...
    func configureListener1()
    {
        listnerCardNum.affinityCalculationStrategy = .prefix
        listnerCardNum.affineFormats = ["[0000] [0000] [0000] [0000]"]
        
        listnerExpiryDate.affinityCalculationStrategy = .prefix
        listnerExpiryDate.affineFormats = ["[00]/[00]"]
    }
    ...
    func generateToken(_ cardParams: STPCardParams)
    {
        STPAPIClient.shared.createToken(withCard: cardParams) { (token: STPToken?, error: Error?) in
            ...
            print(token.tokenId)
            self.billPayment(token.tokenId)
        }
    }

    func billPayment(_ token: String)
    {
        if self.statusCheck == true {
            Api.shared.stripePayment(self, self.paramBillPayment(token)) { responseData in
...
//                self.addPayment(swiftyJsonVar["result"]["id"].stringValue)
    ...
    func paramBillPayment(_ tokenId: String) -> [String : AnyObject]
    {
        var dict: [String : AnyObject] = [:]
        
        dict["user_id"]                = k.userDefault.value(forKey: k.session.userId)! as AnyObject
        dict["bill_id"]                = self.bill_id as AnyObject
